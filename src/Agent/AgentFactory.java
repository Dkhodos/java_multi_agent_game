    ...
    public static Agent createAgent(GameType gameType, int id, int numberOfAgents, Mailer mailer, List<Integer> neighbors, BoSAgentSex bosAgentSex) {
        ...
        return switch (gameType) {
            ...
            case BoS -> {
                ...
                yield new BoSAgent(id, numberOfAgents, mailer, neighbors, bosAgentSex);
            }
        };
    }

    public static Agent[] createAgents(GameType gameType, int numberOfAgents, Mailer mailer, AgentNetwork network, int fraction) {
        ...
        for (int i = 0; i < numberOfAgents; i++) {
            ...
            agents[i] = switch (gameType) {
                case PD -> createAgent(GameType.PD, i, numberOfAgents, mailer, neighbors, null);
                case BoS -> {
                    BoSAgentSex bosAgentSex = createdAgents < fraction ? BoSAgentSex.WIFE : BoSAgentSex.HUSBAND;
                    createdAgents++;
                    yield createAgent(GameType.BoS, i, numberOfAgents, mailer, neighbors, bosAgentSex);
                }
            };
